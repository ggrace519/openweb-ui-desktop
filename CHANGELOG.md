...
- **Cloudflare Access / SSO login stays in the app.** Guest webviews sent every cross-origin navigation to the system browser so chat links would not leave Open WebUI (#165). That also ejected Access and IdP logins, so the session cookie never landed in the webview. SSO navigations now stay in the connection partition; `target="_blank"` chat links still open externally.
